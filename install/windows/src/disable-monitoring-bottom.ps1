...
Write-Host 'Welcome to the monitoring disabling process of TigerSafe for Windows.'
...
try {
    $confirm = Read-Host "`nYou are about to disable monitoring of processes by TigerSafe (WARNING: this is not recommended, but can be needed in some execution environments). Please type `"y`" to disable monitoring, or anything else to cancel (then press Enter to validate)"
    if ($confirm -ne 'y') {
        ExitError "Cancelled disabling of monitoring of TigerSafe."
    }
    ...
    try {
        ...
        $argsToRemove = @('tigersafe.monitoringPeriodMs', 'tigersafe.monitoringProcessesCommandFile', 'tigersafe.monitoringMinProcessesNum', 'tigersafe.monitoringMaxProcessesNum')

        foreach ($argToRemove in $argsToRemove) {
            $jvmArgsLine = RemoveJvmArg $argToRemove $jvmArgsLine
        }
